...
class Security_header {
	...
	public function generate()
	{
		...
		if($this->config['access_control_allow_origin'] !== false) {
			$this->_CI->output->set_header('Access-Control-Allow-Origin:'.$this->config['access_control_allow_origin']);
		}
		...
		if($this->config['expect_staple'] !== false) {
			$this->_CI->output->set_header('Expect-Staple:'.$this->config['expect_staple']);
		}
